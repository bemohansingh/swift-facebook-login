        ...
        loginManager.logIn(permissions: ["email"], from: controller) {[weak self] (loginResult, error) in
            ...
            if let error = error {
                ...
                if let token = result.token {
                    ...
                    self.facebookResponse.send(.failure(FacebookAuthError.cancelled))
                } else {
                    self.facebookResponse.send(.failure(FacebookAuthError.custom("Unable to process Facebook request. Please try again later.")))
                }
            } else {
                self.facebookResponse.send(.failure(FacebookAuthError.custom("Unable to process Facebook request. Please try again later.")))
            }
        ...
        GraphRequest(graphPath: "me", parameters: ["fields": "email,first_name,last_name"]).start { [weak self] (connection, data, error) in
            ...
            if let error = error {
                ...
                guard let data = data as? [String: Any] else {
                    self.loginManager.logOut()
                    self.facebookResponse.send(.failure(FacebookAuthError.custom("Unable to login with facebook at the moment. Please try again later.")))
                    return
                }
